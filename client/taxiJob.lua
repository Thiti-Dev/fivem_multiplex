	...
	while true do
		...
		if onJob == 0 then
			if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
                if IsVehicleModel(GetVehiclePedIsUsing(GetPlayerPed(-1)), GetHashKey("taxi", _r)) or IsVehicleModel(GetVehiclePedIsUsing(GetPlayerPed(-1)), GetHashKey("taxiesperanto", _r)) then
                    if playerData.JOB == "Taxi Driver" then
                        DrawMissionText('Press ~r~E ~w~to start working', 10000)
						...
						if IsPedFatallyInjured(jobs.peds[1]) then
							...
							if jobs.flag[1] == 3 then
								if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), jobs.coords.cx[jobs.flag[2]],jobs.coords.cy[jobs.flag[2]],jobs.coords.cz[jobs.flag[2]], true) > 4.0001 then
                                    ...
                                    --moneyPay(tonumber(disttom)) -- pay
                                    math.randomseed(GetGameTimer())
                                    math.random(); math.random(); math.random()
                                    local randomPay = math.random(15,45)
									moneyPay(randomPay)
									earnExp(11.5)
